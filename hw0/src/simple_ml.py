...
def parse_mnist(image_filename, label_filename):
    ...
    with gzip.open(image_filename, 'rb') as f:
        ...
        X = data.reshape(num_images, rows*cols).astype(np.float32) / 255.0
...
def softmax_loss(Z, y):
    ...
    loss = -np.sum(np.log(softmax_probs[np.arange(num_examples), y])) / num_examples
    return loss
    ### END YOUR CODE
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(num_batches):
        ...
        # Compute logits and softmax probabilities for minibatch
        Z = X_batch.dot(theta)
        exp_Z = np.exp(Z)
        softmax_probs = exp_Z / np.sum(exp_Z, axis=1, keepdims=True)

        # Compute gradient of loss w.r.t. theta for minibatch
        Y = np.zeros((batch, y.max() + 1))
        Y[np.arange(batch), y_batch] = 1
        grad = X_batch.T @ (softmax_probs - Y)
