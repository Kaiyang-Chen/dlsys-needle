...
class PowerScalar(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ### BEGIN YOUR SOLUTION
        return  array_api.power(node.inputs[0], self.scalar-1) * out_grad * self.scalar
        ### END YOUR SOLUTION
...
class EWiseDiv(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ### BEGIN YOUR SOLUTION
        return out_grad / node.inputs[1], out_grad * -1 * node.inputs[0] * array_api.power(node.inputs[1], -2)
        ### END YOUR SOLUTION
...
class BroadcastTo(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ### BEGIN YOUR SOLUTION
        ori_shape = list(node.inputs[0].shape)
        axes = []
        shape = [1] * (len(self.shape) - len(ori_shape)) + ori_shape
        for i, s in enumerate(self.shape):
            if i >= len(shape) or s != shape[i]:
                axes.append(i)
        return reshape(summation(out_grad, tuple(axes)), ori_shape)
        ### END YOUR SOLUTION
